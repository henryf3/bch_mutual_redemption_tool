...
const NOMINAL_UNITS = Number(process.env.NOMINAL_UNITS);

// Set the contract duration in seconds, after which the contract is matured.

const CONTRACT_DURATION_IN_SECONDS = BigInt(Number(process.env.CONTRACT_DURATION_IN_SECONDS));
...
const CONTRACT_LOW_LIQUIDATION_PRICE_MULTIPLIER = Number(process.env.LOW_LIQUIDATION_MULTIPLIER);
const CONTRACT_HIGH_LIQUIDATION_PRICE_MULTIPLIER = Number(process.env.HIGH_LIQUIDATION_MULTIPLIER);
...
const SHORT_WIF = process.env.PEER1_SHORT_P_KEY;
const LONG_WIF = process.env.PEER2_LONG_P_KEY;
...
const example = async function () {
    ...
    try {
        ...
        // Log the next steps.
        console.log(`Wait for ${CONTRACT_DURATION_IN_SECONDS} seconds(s) and the redemption service should mature your contract, paying out to short (${shortPayoutAddress}) and long (${longPayoutAddress}).`);
    }
    catch (error) {
